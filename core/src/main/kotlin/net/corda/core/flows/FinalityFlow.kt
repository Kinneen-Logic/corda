 ...
 * @param expectedTxId
 * @param onlyRelevantStates
 */
class ReceiveFinalityFlow @JvmOverloads constructor(val otherSideSession: FlowSession,
                                                    val expectedTxId: SecureHash? = null,
                                                    val onlyRelevantStates: Boolean = true) : FlowLogic<SignedTransaction>() {
    @Suspendable
    override fun call(): SignedTransaction {
        val stx = subFlow(ReceiveTransactionFlow(otherSideSession, statesToRecord = if (onlyRelevantStates) ONLY_RELEVANT else ALL_VISIBLE))
